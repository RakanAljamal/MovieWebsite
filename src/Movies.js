...
export default function Movies() {
    ...
    useEffect(() => {
        let getMovies = async () => {
            axios.get(`http://aljamal.club/api/page?page=${page}&size=${MAXIMUM_NUMBER_OF_PAGES}`).then(result => {
    ...
    let deleteMovie = (id) => {
        axios.delete(`http://aljamal.club/api/movie/${id}`)
    ...
    inputFile && inputFile.addEventListener('change', function () {
        ...
        axios({
            url: 'http://aljamal.club/api/upload',
            method: "POST",
            data: formData
        }).then(res => {
        })
